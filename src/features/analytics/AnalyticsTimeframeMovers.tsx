...
import type {
  ...
  AnalyticsTimeframeMoversBucket,
} from "./types";
...
const TIMEFRAME_OPTIONS: AnalyticsMoversTimeframe[] = [
  "m",
  "15m",
  "30m",
  "1h",
...
export function AnalyticsTimeframeMovers() {
  ...
  const { data, isLoading, isError } = useGetTimeframeMovers({
    threshold,
    lookbackDays,
    timeframe,
  });
  ...
  return (
    <>
      <div style={{ marginBottom: 20, marginTop: 32 }}>
        ...
        >
          Assets moving ±{threshold}% on {formatTimeframeLabel(timeframe)}{" "}
          candles · hourly distribution
        </p>
      </div>
      <div
        style={{
        ...
        <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <select
              value={timeframe}
              onChange={(e) =>
                setTimeframe(e.target.value as AnalyticsMoversTimeframe)
              }
              style={{
                fontFamily: "var(--font-mono)",
                fontSize: "0.6rem",
            ...
            <select
              ...
              onChange={(e) =>
                setThreshold(
                  Number(e.target.value) as (typeof THRESHOLDS)[number],
                )
              }
              style={{
                fontFamily: "var(--font-mono)",
            ...
            <>
              ...
              >
                {hourlyData.map((d) => {
                  const gH = Math.max(
                    d.gainers > 0 ? 2 : 0,
                    Math.round((d.gainers / maxGainers) * (halfH - 6)),
                  );
                  const lH = Math.max(
                    d.losers > 0 ? 2 : 0,
                    Math.round((d.losers / maxLosers) * (halfH - 6)),
                  );
                      ...
                      {gH > 0 && (
                        <div
                          style={{
                            ...
                            boxShadow: isActive
                              ? "0 0 10px oklch(0.68 0.20 145 / 35%)"
                              : "none",
                      ...
                      {lH > 0 && (
                        <div
                          style={{
                            ...
                            boxShadow: isActive
                              ? "0 0 10px oklch(0.58 0.20 25 / 35%)"
                              : "none",
                            transition: "background 0.12s, box-shadow 0.12s",
                            animationName: "barGrow",
                            animationDuration: "0.55s",
              ...
              {/* Tooltip */}
              {activeData &&
                (activeData.gainers > 0 || activeData.losers > 0) && (
                  <div
                    style={{
                      position: "absolute",
                      top: 8,
          ...
          <div className="flex items-center gap-1.5">
            <div
              style={{
                ...
                height: 8,
                borderRadius: 2,
                background: "oklch(0.52 0.16 145 / 75%)",
              }}
            />
            <span
              style={{
          ...
          <div className="flex items-center gap-1.5">
            <div
              style={{
                ...
                height: 8,
                borderRadius: 2,
                background: "oklch(0.46 0.16 25 / 75%)",
              }}
            />
            <span
              style={{
